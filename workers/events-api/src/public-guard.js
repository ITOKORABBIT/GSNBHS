...
export async function claimPublicDedupe(env, scope, parts, ttlSeconds) {
  const dedupeKey = await sha256Key([scope, ...parts.map((part) => text(part))].join("|"));
  const now = Math.floor(Date.now() / 1000);
...
async function publicSubjectKey(request, scope) {
  const forwarded = text(request?.headers?.get("CF-Connecting-IP") || request?.headers?.get("X-Forwarded-For"));
