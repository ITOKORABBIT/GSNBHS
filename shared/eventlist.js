...
function cardHTML(e){
  ...
  const statusOptions=['草稿','報名中','已截止','已結束','已取消'].map(s=>`<option${s===e.status?' selected':''}>${s}</option>`).join('');
  return `
