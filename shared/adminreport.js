...
function togglePublicFields() {
  document.getElementById('publicFields').classList.toggle('open', document.getElementById('f_public').checked);
}
...
document.getElementById('reportForm').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    var payload = {
      ...
      publicFlag: isPublic,
      publicTitle: isPublic ? (document.getElementById('f_pub_title').value.trim() || title) : '',
      publicCate: isPublic ? document.getElementById('f_cate').value : '',
      publicLoc: isPublic ? (document.getElementById('f_pub_loc').value.trim() || addr) : '',
      publicSummary: isPublic ? document.getElementById('f_pub_summary').value.trim() : ''
    };
    ...
    if (json.caseId) {
      ...
      if (successText) successText.textContent = '案件編號：' + json.caseId + (isPublic ? '（已設為公開）' : '');
    }
    ...
    showToast('送出失敗：' + (ERROR_MSGS[err.message] || err.message || '未知錯誤'));
    btn.disabled = false;
    btn.innerHTML = '<svg class="icon"><use href="#i-send"/></svg> 送出通報';
  }
});
