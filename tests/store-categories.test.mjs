...
const OTHER_VILLAGE_CATES = ["住宅相關", "寵物專區", "里內日常小吃", "家庭好友聚餐", "大坑名產貴賓招待"];

function read(name) {
  return fs.readFileSync(new URL("../" + name, import.meta.url), "utf8");
}

test("本里後端的預設分類就是里長那七類", () => {
...
test("本里的分類清單寫在自己的 config，不寫死在四里共用的程式裡", () => {
  const config = read("config.js");
  assert.match(config, /STORE_CATEGORIES: \[/);
  for (const cate of CATEGORIES) assert.ok(config.includes("'" + cate + "'"), "config 少了「" + cate + "」");

  // 共用檔一律讀 CONFIG 或該里 API 的 taxonomy，不能再出現任何一個里的分類名稱
  for (const file of ["shared/storelist.js", "shared/storeopenlist.js", "shared/storedetail.js", "shared/store.js"]) {
    const src = read(file);
    for (const name of ["飲料冰品", "健康醫療", "生活便利", "學術教育", "運動休閒", ...OTHER_VILLAGE_CATES]) {
      assert.ok(!src.includes(name), file + " 仍寫死分類「" + name + "」");
    }
    assert.match(src, /CONFIG\.STORE_CATEGORIES/, file + " 沒有讀 config 的分類清單");
  }
...
test("LINE 美食地圖選單跟著換成新分類（本里 Worker，各里各自維護）", () => {
